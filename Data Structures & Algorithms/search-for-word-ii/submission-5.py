...
class Solution:
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        ...
        def dfs(r,c,word,node): # node is trienode
            ...
            visited.add((r,c)) # mutable，共享 → 要 undo
            word += board[r][c] # string, immutable, 不要undo
            node = node.children[board[r][c]] # 易错点， 在这行之前，node是上一级的，因为我们从root出发 # node = xx 只是一个指针，是个local variable，所以之后不用undo

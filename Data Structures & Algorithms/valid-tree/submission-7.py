class Solution:
    def validTree(self, n: int, edges: List[List[int]]) -> bool:
        # time: O( V + E)
        # spcae: O( V + E)
        # Undirected graph is a tree iff 1） it is connected and 2） has no cycle.
        ...
        def dfs(node,prev):
            if node in visited:
                return False
            # if not node: # 易错点：这里写了就是错。如果node = 0，就是错的；这里的node不是tree node! 只是一个integer!!!
            #     return True
            
            #process current node
            visited.add(node)
            for nei in node2nei[node]:
                if nei == prev: # don't repeat the way back，不要检查父节点
                    continue 
                if not dfs(nei,node):
                    return False
            return True

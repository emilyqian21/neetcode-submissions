...
class Solution:
    ...
    #Bucket sort achieves optimal O(N) time complexity, but requires O(N) extra space because we allocate a bucket for every possible frequency. The heap solution runs in O(N + M log k) time but only needs O(M + k) space. In practice, when k is small compared to N, I would usually choose the heap solution because it is more memory-efficient and scales better for large datasets.

...
#         self.next = next
        ...
        # find mid point (use even list as example because for odd list, f = head or f = head.next give the same result)
        f = head.next
        s = head
        # Odd length: no difference.
        # Even length: fast = head.next makes slow stop at the left-middle.
        while f and f.next:
            f = f.next.next
            s = s.next
        first = head

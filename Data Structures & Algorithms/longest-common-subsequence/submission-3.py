class Solution:
    def longestCommonSubsequence(self, text1: str, text2: str) -> int:
        ...
        # definition: dp[i][j] = at text1[:i] and text2[:j], what is longest common subsequence so far
        # main logic: 假设text1[i - 1]和text2[j - 1]一样，那就 dp[i -1][j - 1]加 1； 如果不相同，那就选择是用text1[i - 1]还是text2[j - 1]
        n = len(text1)
        m = len(text2)

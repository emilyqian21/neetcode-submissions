    ...
    def numDecodings(self, s: str) -> int:
        ...
        for i in range(1, n + 1):
            ...
            if s[i-1] != "0": # s[i-1] is the number before current number; if the number before current number is not 0
                curr += prev1 # the ways to decode before current number = ways to decode 1 digit before current number 
            #two digit
            if i >= 2 and ((s[i-2] =="1") or (s[i-2] == "2" and s[i-1] in "0123456")): # 10 <= s[i-2]s[i-1] <=26
                curr += prev2
            prev2 = prev1
            prev1 = curr
        return curr

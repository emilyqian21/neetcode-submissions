class Solution:
    def foreignDictionary(self, words: List[str]) -> str:
        ...
        # - E = number of precedence relationships

        # Building the graph takes O(C), and the DFS-based topological sort visits each character and edge once.

        # Space Complexity:
        # O(V + E)

        #for the graph, visited/status map, recursion stack, and output.
        ...
        # topolocigal sort
        status = {} # True = visiting and in the loop  # False = visited but not in the loop
        output = []
        def dfs(c): 
            # return style, dfs(c) means if there is loop. Loop -> True, no loop -> False
            # base case
            if c in status:

class Solution:
    def combinationSum2(self, candidates: List[int], target: int) -> List[List[int]]:
        # Time: O(n × 2ⁿ) because each candidate can be either selected or skipped, resulting in up to 2ⁿ subsets, and copying each valid combination costs O(n).

        # Space: auxiliary O(n) --> recurssion stack
                # output -->  O(n × 2ⁿ).
